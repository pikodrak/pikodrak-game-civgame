...
def generate_earth_map(width, height, seed=None):
    ...
    def is_land(nx, ny):
        ...
        # North America (nx 0.05-0.25, ny 0.15-0.55)
        if 0.05 < nx < 0.25 and 0.15 < ny < 0.55:
            if nx < 0.08 and ny > 0.4: return False  # Pacific coast indent
            if ny < 0.2 and nx > 0.2: return True  # Canada/Alaska
            if 0.1 < nx < 0.22 and 0.2 < ny < 0.5: return True  # Main continent
            if 0.12 < nx < 0.18 and 0.48 < ny < 0.55: return True  # Central America
            return ny < 0.45 and nx > 0.08
        # South America (nx 0.15-0.30, ny 0.55-0.90)
        if 0.15 < nx < 0.32 and 0.55 < ny < 0.92:
            cx, cy = 0.22, 0.72
            dx, dy = (nx - cx) / 0.1, (ny - cy) / 0.2
            return dx * dx + dy * dy < 1.2
        # Europe (nx 0.42-0.55, ny 0.15-0.38)
        if 0.42 < nx < 0.56 and 0.15 < ny < 0.40:
            if ny < 0.2: return nx > 0.48  # Scandinavia
            return True
        # Africa (nx 0.42-0.58, ny 0.38-0.80)
        if 0.42 < nx < 0.60 and 0.38 < ny < 0.82:
            cx, cy = 0.50, 0.58
            dx, dy = (nx - cx) / 0.1, (ny - cy) / 0.22
            return dx * dx + dy * dy < 1.3
        # Asia (nx 0.55-0.85, ny 0.10-0.50)
        if 0.55 < nx < 0.88 and 0.10 < ny < 0.52:
            if ny < 0.15: return nx > 0.65 and nx < 0.82  # Siberia
            if nx > 0.82 and ny > 0.35: return False  # Pacific
            return True
        # India (nx 0.62-0.70, ny 0.42-0.58)
        if 0.62 < nx < 0.72 and 0.42 < ny < 0.58:
            return True
        # Southeast Asia / Indonesia (nx 0.72-0.88, ny 0.48-0.60)
        if 0.72 < nx < 0.88 and 0.48 < ny < 0.62:
            return random.random() < 0.4  # Islands
        # Australia (nx 0.78-0.90, ny 0.65-0.80)
        if 0.78 < nx < 0.92 and 0.65 < ny < 0.82:
            cx, cy = 0.85, 0.73
            dx, dy = (nx - cx) / 0.07, (ny - cy) / 0.08
            return dx * dx + dy * dy < 1.0
        # Greenland (nx 0.28-0.35, ny 0.08-0.20)
        if 0.28 < nx < 0.36 and 0.08 < ny < 0.22:
            return True
        # Japan (nx 0.84-0.88, ny 0.25-0.38)
        if 0.84 < nx < 0.89 and 0.25 < ny < 0.38:
            return random.random() < 0.5
        # UK (nx 0.43-0.46, ny 0.18-0.25)
    ...
    for q in range(width):
        for r in range(height):
            nx = q / width
            ny = r / height
            # Add noise for natural coastlines
    ...
    for q in range(width):
        for r in range(height):
            ...
            is_coast = any(tiles.get((nq, nr), Terrain.WATER) in (Terrain.WATER, Terrain.COAST)
                          for nq, nr in hex_neighbors(q, r) if 0 <= nq < width and 0 <= nr < height)
            tiles[(q, r)] = pick_terrain(nx, ny, False)

    # Sahara desert band (ny ~0.35-0.45, nx 0.42-0.58)
    for q in range(width):
        for r in range(height):
            nx, ny = q / width, r / height
    ...
    # Rockies, Andes, Alps, Himalayas, Urals
...
class GameState:
    def __init__(self, width=40, height=30, num_players=4, seed=None, map_type="random", wrap=False):
        self.width = width
        self.height = height
